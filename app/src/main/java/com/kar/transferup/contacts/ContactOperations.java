...
public class ContactOperations {
    ...
    public ContactOperations(Context context, long contactId, String userId, String accountName, boolean isSyncOperation, BatchOperation batchOperation) {
        this(context, isSyncOperation, batchOperation);
        mBackReference = mBatchOperation.size();
        mIsNewContact = true;
        //mValues.put(RawContacts.CONTACT_ID, contactId);
     ...
     */
    public static ContactOperations createNewContact(Context context, long contactId, String userId,
                                                     String accountName, boolean isSyncOperation, BatchOperation batchOperation) {
        return new ContactOperations(context, contactId, userId, accountName, isSyncOperation, batchOperation);
    }
    /**
    ...
    public ContactOperations addAvatar(String avatarUrl) {
        if (avatarUrl != null) {
            byte[] avatarBuffer = NetworkUtils.downloadAvatar(avatarUrl);
     ...
     *
     * @param contact the userId of the sample SyncAdapter user object
     * @return instance of ContactOperations
     */
    public ContactOperations addProfileAction(Contact contact) {
        mValues.clear();
        if (contact.getServerId() != null) {
            mValues.put(TransferUpSyncAdapterColumns.DATA_PID, contact.getPhoneNumbers().get(0).getNumber());
            mValues.put(Data.MIMETYPE, TransferUpSyncAdapterColumns.MIME_PROFILE);
            mValues.put(TransferUpSyncAdapterColumns.DATA_SYNC, contact.getServerId());
            mValues.put(TransferUpSyncAdapterColumns.DATA_SUMMARY, mContext
                .getString(R.string.profile_action));
            mValues.put(TransferUpSyncAdapterColumns.DATA_DETAIL, mContext
     ...
     * @return instance of ContactOperations
     */
    public ContactOperations addProfileAction(RawContact contact) {
        mValues.clear();
        if (contact.getServerContactId() != null) {
            mValues.put(TransferUpSyncAdapterColumns.DATA_PID, contact.getServerContactId());
            mValues.put(Data.MIMETYPE, TransferUpSyncAdapterColumns.MIME_PROFILE);
            mValues.put(TransferUpSyncAdapterColumns.DATA_SYNC, contact.getServerContactId());
            mValues.put(TransferUpSyncAdapterColumns.DATA_SUMMARY, mContext
                .getString(R.string.profile_action));
            mValues.put(TransferUpSyncAdapterColumns.DATA_DETAIL, mContext
                .getString(R.string.view_profile));
    ...
    public ContactOperations updateProfileAction(Integer userId, Uri uri) {
        mValues.clear();
        mValues.put(TransferUpSyncAdapterColumns.DATA_PID, userId);
        addUpdateOp(uri);
        return this;
    }
    /**
    ...
    public ContactOperations addEmail(List<com.kar.transferup.contacts.Email> emails) {
        mValues.clear();
        for(com.kar.transferup.contacts.Email email : emails) {
            mValues.put(Email.DATA, email.getAddress());
            mValues.put(Email.TYPE, email.getType().name());
            mValues.put(Email.MIMETYPE, Email.CONTENT_ITEM_TYPE);
            addInsertOp();
        }
        return this;
    ...
    public ContactOperations addPhones(List<PhoneNumber> phoneNumbers) {
        mValues.clear();
        for(PhoneNumber phone : phoneNumbers) {
            mValues.put(Phone.NUMBER, phone.getNumber());
            mValues.put(Phone.TYPE, phone.getType().name());
            mValues.put(Phone.MIMETYPE, Phone.CONTENT_ITEM_TYPE);
            addInsertOp();
        }
        return this;
